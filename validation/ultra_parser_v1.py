"""Ультра-агрессивный парсер для извлечения данных из чеков."""
import re
import yaml
...
class UltraReceiptParser:
    """Ультра-агрессивный парсер чеков с максимальным поиском."""
    ...
    def _compile_patterns(self):
        ...
        # Ультра-агрессивные паттерны для телефонов
        phone_patterns = [
            # Основные паттерны
        ...
        # Ультра-агрессивные паттерны для сумм
        amount_patterns = [
            # Основные паттерны
            r'(\d+(?:[.,]\d{2})?)\s*(?:руб|₽|rub|р\.|Р)',
            r'(?:сумма|итого|к оплате)[\s:]*(\d+(?:[.,]\d{2})?)',
            r'(\d+(?:[.,]\d{2})?)\s*(?:€|eur|euro)',
            r'(\d+(?:[.,]\d{2})?)\s*(?:\$|usd|dollar)',

            # Дополнительные паттерны
            r'(?:перевод|платеж|оплата)[\s:]*(\d+(?:[.,]\d{2})?)',
            r'(?:сумма перевода|сумма к переводу|к переводу)[\s:]*(\d+(?:[.,]\d{2})?)',
            r'(?:сумма к оплате|итого к оплате)[\s:]*(\d+(?:[.,]\d{2})?)',

            # Специальные паттерны для OCR ошибок
            r'(\d+)\s*Р',  # 1500Р
            r'(\d+)\s*р',  # 1500р
            r'(\d+)\s*руб',  # 1500руб
            r'(\d+)\s*₽',  # 1500₽

            # Поиск любых чисел с копейками
            r'(\d+[.,]\d{2})',
            r'(\d+\.\d{2})',
            r'(\d+,\d{2})',

            # Поиск целых чисел (возможные суммы)
            r'(\d{3,6})',  # От 100 до 999999
        ]
    ...
    def parse_receipt(self, text: str, confidence: float = 0.0) -> ParsedData:
        ...
        print(f"🔍 Начинаем ультра-парсинг чека (уверенность OCR: {confidence:.1f}%)")
        ...
        parsed_data.recipient_phone = self._extract_phone_ultra(text)

        # Извлекаем сумму
        parsed_data.amount = self._extract_amount_ultra(text)
        ...
        print(f"📊 Результат ультра-парсинга:")
    ...
    def _extract_phone_ultra(self, text: str) -> Optional[str]:
        """Ультра-агрессивно извлекает номер телефона из текста."""
        print("🔍 Ультра-поиск номера телефона...")
                ...
                if all(len(part) >= 2 for part in potential_parts):
                    ...
                    if len(combined) >= 10:
                        ...
                        if len(combined) == 11:
                            ...
                            if normalized:
                                # Дополнительная проверка - номер должен быть в валидных
                                ...
                                if normalized in valid_phones:
                                    ...
                                    print(f"❌ Номер по частям не валидный: {normalized}")
                                    continue

        print("❌ Номер телефона не найден")
        return None

    def _extract_amount_ultra(self, text: str) -> Optional[float]:
        """Ультра-агрессивно извлекает сумму из текста."""
        print("🔍 Ультра-поиск суммы...")
        ...
        for i, pattern in enumerate(self.compiled_patterns['amount']):
            matches = pattern.findall(text)
            if matches:
                for match in matches:
                    if isinstance(match, tuple):
                    ...
                    if amount:
                        print(f"✅ Найдена сумма по паттерну {i+1}: {amount}")
                        return amount

        # Ультра-агрессивный поиск - ищем все числа с копейками
        print("🔍 Ультра-поиск - все числа с копейками...")
        decimal_numbers = re.findall(r'\d+[.,]\d{2}', text)
        for number_str in decimal_numbers:
            amount = self._parse_amount_value(number_str)
            if amount and self._is_reasonable_amount(amount):
                print(f"✅ Найдена сумма в числах с копейками: {amount}")
                return amount

        # Поиск целых чисел (возможные суммы)
        print("🔍 Поиск целых чисел...")
        whole_numbers = re.findall(r'\b\d{3,6}\b', text)
        for number_str in whole_numbers:
            amount = self._parse_amount_value(number_str)
            if amount and self._is_reasonable_amount(amount):
                print(f"✅ Найдена сумма в целых числах: {amount}")
                return amount
    ...
    def _is_reasonable_amount(self, amount: float) -> bool:
        """Проверяет, является ли сумма разумной."""
        # Проверяем, что сумма в разумных пределах
        if 1 <= amount <= 1000000:  # От 1 рубля до 1 миллиона
            # Дополнительная проверка - исключаем явно неподходящие числа
            # Исключаем числа, которые могут быть датами (13, 36, 2025, etc.)
            if amount in [13, 36, 2025, 2024, 2023, 2022, 2021, 2020]:
                return False
            # Исключаем числа, которые могут быть временем (18, 58, 49)
            if amount in [18, 58, 49, 19, 20, 21, 22, 23, 24]:
                return False
            return True
        return False
